  ...
  (function () {
    ...
    if (window.matchMedia("(max-width:768px)").matches) {
      ...
      function getX(idx) {
        ...
        if (!item) return 0;
        return strip.offsetWidth / 2 - item.offsetLeft - item.offsetWidth / 2;
      }

      function goTo(idx, animate) {
        if (idx >= total * 2) idx -= total;
        else if (idx < total) idx += total;
        current = idx;
        track.style.transition =
          animate === false ? "none" : "transform .52s cubic-bezier(0.16,1,0.3,1)";
      ...
      dots.forEach(function (dot) {
        dot.addEventListener("click", function () {
          goTo(total + parseInt(dot.dataset.index));
          startAuto();
        });
      });
